...
class userGame : protected ReactionGame{
  ...
  void choice_user_himself(int* user_id, int rhs); 
...
void userGame::choice_user_himself(int* user_id, int rhs) {
  rhs = any_choice_with_button(user_id, 3);
  user_id = rhs;
}
    ...
    while (true) {
      ...
      if(currentTime - lastChangeTime > cycleDelay) {
        for(int i = 0; i < optionsCount; i++) {
          digitalWrite(ledPins[i], i == currentOptions ? HIGH : LOW);
        }
        lastChangeTime = currentTime;
        currentOptions = (currentOptions + 1) % optionsCount;
      }
...
void ReactionGame::setLevel(int* newLevel) {
  ...
  if(newLevel >= 1 && newLevel <= 3) {
    int rhs = any_choice_with_button(newLevel, 3);
    level = rhs;
