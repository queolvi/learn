...
double IPO8381::Matrix::Norm_sq() const {
  double sum = 0.0;
  for(size_t i = 0; i < nrows(); ++i) {
    for(size_t j = 0; j < ncols(); ++j) {
      sum += arr_[i][j] * arr_[i][j];
    }
  }
  return std::sqrt(sum);
}

double IPO8381::Matrix::vector_norm(const std::vector<double> v) {
  double sum = 0.0;
  for(double val : v) {
    sum += val * val;
  }
...
std::vector<double> IPO8381::Matrix::mult_vector(const std::vector<double> v) const {
  if (ncols() != v.size()) {  throw std::invalid_argument("Size no matched! ");}
  std::vector<double> res(nrows(), 0.0);
  for(size_t i = 0; i < nrows(); ++i) {
    for(size_t j = 0; j < ncols(); ++j) {
      res[i] += (*this)[i][j] * v[j];
    }
...
IPO8381::Matrix IPO8381::Matrix::transpose() const {
  IPO8381::Matrix res(ncols(), nrows());
  for(size_t i = 0; i < nrows(); ++i) {
    for(size_t j = 0; j < ncols(); ++j) {
      res[j][i] = (*this)[i][j];
    }
